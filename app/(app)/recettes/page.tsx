...
export default function RecettesPage() {
  ...
  // Combine sample + DB recipes
  const allRecipes = [...SAMPLE_RECIPES, ...recipes.map(r => ({
    id: r.id,
    name: r.name,
    cal: r.ingredients ? Math.round(calculateNutrition(r).calories) : 0,
    prot: r.ingredients ? Math.round(calculateNutrition(r).protein * 10) / 10 : 0,
    carbs: r.ingredients ? Math.round(calculateNutrition(r).carbs * 10) / 10 : 0,
    fat: r.ingredients ? Math.round(calculateNutrition(r).fat * 10) / 10 : 0,
    time: r.prep_time || 0,
    tags: r.tags || [],
    emoji: '🍽️',
  ...
  const handleCreateRecipe = async () => {
    ...
    await createRecipe({ name: recipeForm.name, servings: recipeForm.servings })
      ...
      {showNewRecipe && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ ...DF, fontSize: 18, fontWeight: 900, color: WHEAT, marginBottom: 16 }}>Nouvelle recette</p>
            <input type="text" placeholder="Nom de la recette" value={recipeForm.name}
              onChange={e => setRecipeForm({ ...recipeForm, name: e.target.value })}
              style={{
                width: '100%', padding: '10px 12px', borderRadius: 8, background: 'var(--bg-input)',
                border: '1px solid var(--border)', color: WHEAT, marginBottom: 12, boxSizing: 'border-box'
              }} />
            <input type="number" placeholder="Portions" value={recipeForm.servings}
              onChange={e => setRecipeForm({ ...recipeForm, servings: parseInt(e.target.value) || 1 })}
              style={{
                width: '100%', padding: '10px 12px', borderRadius: 8, background: 'var(--bg-input)',
                border: '1px solid var(--border)', color: WHEAT, marginBottom: 16, boxSizing: 'border-box'
              }} />
        ...
        <div style={{ gridColumn: 'span 2', padding: '26px 28px 22px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'hidden' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
              <button className="rec-btn" onClick={() => setShowNewRecipe(!showNewRecipe)}
