...
function Kbd({ k }: { k: string }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 26, height: 22, padding: '0 6px', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 5, fontSize: 10, color: 'var(--text)', ...DF, fontWeight: 700, boxShadow: 'var(--elev-1)' }}>
      {k}
    </span>
  )
}
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 640, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {SHORTCUTS.map(cat => (
          <div key={cat.category} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--elev-1)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
