...
function Toggle({ value, onChange, label, sub, icon: Icon, color }: {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: '1px solid var(--border)' }}>
      <div style={{ width: 36, height: 36, borderRadius: 10, background: value ? color + '18' : 'var(--bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '1px solid var(--border)' }}>
      ...
      <button onClick={() => onChange(!value)} className="nb-press"
        style={{ width: 42, height: 24, borderRadius: 99, background: value ? color : 'var(--bg-input)', border: '1px solid var(--border)', boxShadow: 'var(--elev-1)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
...
export default function NotificationsPage() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 600, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--elev-1)', borderRadius: 'var(--radius-lg)', padding: '4px 20px 4px', marginBottom: 16 }}>
        {items.map(item => (
      ...
      <button onClick={save} disabled={saving} className="nb-press"
        style={{ width: '100%', background: ORANGE, color: 'var(--chocolate)', borderRadius: 'var(--radius-lg)', padding: '12px 0', ...DF, fontWeight: 800, fontSize: 12, border: '1px solid var(--border)', boxShadow: 'var(--elev-1)', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.08em', opacity: saving ? 0.6 : 1 }}>
