...
const PRIORITY_COLOR: Record<string, string> = {
  urgent: 'var(--accent-brand)', high: 'var(--text)', medium: 'var(--azul)', low: '#888',
}
...
export default function TodoPage() {
  ...
  const priorityGroups: { key: 'urgent'|'high'|'medium'|'low'; label: string; color: string }[] = [
    { key: 'urgent', label: 'Urgent',  color: 'var(--accent-brand)' },
  ...
  const autoGroups = [
    { key: 'overdue',   label: 'En retard',           color: 'var(--accent-brand)', tasks: filtered.filter(t => t.due_date && t.due_date < today && t.status !== 'done'), bg: 'rgba(242,84,45,0.05)' },
    { key: 'today',     label: 'Aujourd\'hui',         color: 'var(--accent-brand)', tasks: filtered.filter(t => t.due_date === today && t.status !== 'done'),              bg: undefined },
  ...
  return (
    ...
    <div style={{ padding: 30, display: 'flex', flexDirection: 'column', gap: 10, minHeight: '100%' }}>
      ...
      <KpiGrid>
        ...
        <KpiCard label="Urgentes"         value={String(urgentTasks.length)} color="var(--accent-brand)" />
      ...
      <div className="flex gap-3 flex-wrap">
        {tabs.map(t => {
          ...
          const darkTab = t.key === 'priorites' || t.key === 'projets'
          return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 flex flex-col gap-4">
          ...
          {tab === 'toutes' && (
            <>
              <TaskSection title="Aujourd'hui"   color="var(--accent-brand)"           tasks={todayTasks}  onToggle={toggle} onEdit={setEditingTask} loading={loading} projects={projects} />
              ...
              {lateTasks.length > 0 && (
                <TaskSection title="En retard"   color="var(--accent-brand)"           tasks={lateTasks}   onToggle={toggle} onEdit={setEditingTask} loading={loading} projects={projects} bg="rgba(242,84,45,0.05)" />
              )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', border: '2px solid var(--ink)', boxShadow: '4px 4px 0 var(--ink)', padding: 16 }}>
            <p style={{ ...DF, fontSize: 11, fontWeight: 800, letterSpacing: '0.12em', color: 'var(--accent-brand)', textTransform: 'uppercase', marginBottom: 12 }}>Répartition priorités</p>
