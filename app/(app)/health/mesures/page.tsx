...
export default function MesuresPage() {
  ...
  const [form, setForm] = useState({
    ...
    taille: '', massGrasse: '', massMuscul: '', imc: '', notes: '',
  })

  // Local state (future: save to Supabase in health_metrics extended table)
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    const newMesure: Mesure = {
      ...
      massGrasse: form.massGrasse ? parseFloat(form.massGrasse) : undefined,
      massMuscul: form.massMuscul ? parseFloat(form.massMuscul) : undefined,
      imc:        form.imc        ? parseFloat(form.imc)        : undefined,
      notes:      form.notes || undefined,
    }
    setMesures(m => [newMesure, ...m])
    setShowForm(false)
    setForm({ date: new Date().toISOString().slice(0, 10), taille: '', massGrasse: '', massMuscul: '', imc: '', notes: '' })
  }
  ...
  return (
    <div style={{ padding: 30, minHeight: '100%', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
        {metrics.map(m => (
          <div key={m.l} style={{ padding: '22px 20px', borderRadius: 12, background: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {m.vals.length >= 2 && (
              <MiniChart values={m.vals} color={m.color} />
            )}
          </div>
        ))}
      </div>
      ...
      {latest?.imc && (
        <div style={{ padding: '16px 20px', borderRadius: 12, background: 'var(--bg-card)', border: '1px solid var(--border)', marginBottom: 16, display: 'flex', gap: 16, alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 0, height: 16, borderRadius: 99, overflow: 'hidden' }}>
              {[
                { l: 'Sous-poids', max: 18.5, color: '#3B82F6' },
                { l: 'Normal',     max: 25,   color: TEAL },
                { l: 'Surpoids',   max: 30,   color: '#F59E0B' },
                { l: 'Obésité',    max: 40,   color: ORANGE },
      ...
      <div style={{ borderRadius: 12, background: 'var(--bg-card)', border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px 80px',
        ...
        {mesures.map((m, i) => {
          const p = mesures[i + 1]
          return (
            <div key={m.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px 80px',
